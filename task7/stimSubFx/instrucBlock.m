...
switch bkIdx
    case 1
        ...
        msg{2} = sprintf('Each token is worth %d %s%s at first. After some time, the token will "mature" to either a positive or negative value.',...
        ...
        msg{2} = 'Sometimes you might want to sell a token before it matures in order to move on to a new one.';
        ...
        msg{1} = sprintf('%s Your goal is to earn the most %s you can in the available time, by deciding how long to keep waiting for individual tokens. %s',...
            txt1,params.currency);
        msg{2} = txt2;
        txt = sprintf('%s\n\n',msg{:}); txt((end-1):end) = [];
        showMsg(wid,bkgd,txt,respKey);
        ...
        Screen('TextSize',wid,rects.txtsize_msg);
        msg = {};
        msg{1} = 'You should sell tokens quickly when they mature, since their value will not change again.';
        msg{2} = 'Even if a token matures to a negative value, it''s best to sell it quickly in order to move on to the next one.';
        msg{3} = 'Any questions?';
        txt = sprintf('%s\n\n',msg{:}); txt((end-1):end) = [];
        showMsg(wid,bkgd,txt,respKey);
